...
export default function (UIkit) {
    ...
    UIkit.component = function (name, options) {
        ...
        if (UIkit._initialized && !opt.functional) {
            fastdom.read(() => UIkit[name](`[mui-${id}],[data-mui-${id}]`));
        }
...
export const getComponentName = memoize((attribute) => {
    return startsWith(attribute, 'mui-') || startsWith(attribute, 'data-mui-')
        ? camelize(attribute.replace('data-mui-', '').replace('mui-', ''))
        : false;
});
